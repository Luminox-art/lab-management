	...
	SecurityFilterChain securityFilterChain(HttpSecurity http, ObjectMapper objectMapper) throws Exception {
		...
		http.authorizeHttpRequests(authorization -> authorization
				.requestMatchers("/actuator/health/readiness", "/api/v1/auth/register", "/api/v1/auth/login", "/login",
						"/register", "/registration-pending", "/error", "/favicon.ico", "/css/**", "/js/**",
						"/v3/api-docs", "/v3/api-docs.yaml", "/v3/api-docs/**", "/swagger-ui.html", "/swagger-ui/**")
				.permitAll().requestMatchers("/api/v1/users/**", "/admin/users/**").hasAnyRole("CBQL", "ADMIN")
				.requestMatchers(HttpMethod.POST, "/api/v1/rooms", "/api/v1/devices").hasAnyRole("CBQL", "ADMIN")
				.requestMatchers(HttpMethod.PUT, "/api/v1/rooms/**", "/api/v1/devices/**").hasAnyRole("CBQL", "ADMIN")
				.requestMatchers(HttpMethod.POST, "/api/v1/registrations/*/approve", "/api/v1/registrations/*/reject")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers("/api/v1/admin-blocks/**", "/admin-blocks/**")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers("/api/v1/maintenances/**", "/maintenances/**")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers("/api/v1/dashboard/**", "/dashboard/**")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers(HttpMethod.PATCH, "/api/v1/incidents/**")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers(HttpMethod.POST, "/incidents/*/update")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers(HttpMethod.POST, "/api/v1/registrations")
				.hasAnyRole("GV", "SV").requestMatchers(HttpMethod.POST, "/api/v1/registrations/*/cancel")
				.hasAnyRole("GV", "SV").requestMatchers(HttpMethod.PUT, "/api/v1/registrations/**")
				.hasAnyRole("GV", "SV").requestMatchers(HttpMethod.GET, "/registrations/new", "/registrations/*/edit")
				.hasAnyRole("GV", "SV")
				.requestMatchers(HttpMethod.POST, "/registrations/*/approve", "/registrations/*/reject")
				.hasAnyRole("CBQL", "ADMIN").requestMatchers(HttpMethod.POST, "/registrations").hasAnyRole("GV", "SV")
